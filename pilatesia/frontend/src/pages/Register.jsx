...
export default function Register() {
  ...
  const onSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await http.post("/register", { studio_code, full_name, email, password });
          ...
          <div className="card-body p-4 p-md-5">
            ...
            <form onSubmit={onSubmit} className="auth-form">
              <div className="auth-field">
                <label className="form-label" htmlFor="reg-studio">Stüdyo kodu</label>
                <input id="reg-studio" className="form-control form-control-lg" placeholder="Stüdyonuzun kodu (örn: istanbul-pilates)" value={studio_code} onChange={(e) => setStudioCode(e.target.value)} required autoComplete="off" />
                <div className="form-text">Katılacağınız stüdyonun size verilen kodunu girin.</div>
              </div>
              <div className="auth-field">
            ...
            <div className="auth-footer">
              ...
              <Link to="/register-studio" className="auth-link-btn ms-2">Yeni stüdyo aç</Link>
            </div>
          </div>
        </div>
      </div>
