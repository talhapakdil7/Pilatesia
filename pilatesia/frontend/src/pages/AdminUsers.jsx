                ...
                {filteredUsers.map((u) => (
                  <tr key={u.id}>
                    ...
                    <td className="text-end">
                      ...
                      <button className={`btn btn-outline-danger btn-sm ${deletingId === u.id ? "btn-loading" : ""}`} onClick={() => openDeleteConfirm(u)} disabled={deletingId !== null}>
          ...
          <div className="modal-dialog modal-dialog-centered modal-sm">
            <div className="modal-content">
              <div className="modal-header">
                ...
                <button type="button" className="btn-close" onClick={closeDeleteConfirm} aria-label="Kapat" />
              </div>
              <div className="modal-body">
                <p className="mb-0">
                  <strong>{deleteConfirmUser.full_name}</strong> ({deleteConfirmUser.email}) kullanıcısını silmek istediğinize emin misiniz? Bu işlem geri alınamaz.
                </p>
              </div>
              <div className="modal-footer">
                <button type="button" className="btn btn-outline-secondary" onClick={closeDeleteConfirm}>İptal</button>
                <button type="button" className={`btn btn-danger ${deletingId === deleteConfirmUser.id ? "btn-loading" : ""}`} onClick={() => remove(deleteConfirmUser.id)} disabled={deletingId !== null}>
                  {deletingId === deleteConfirmUser.id && <span className="btn-spinner" />}
                  Sil
                </button>
              </div>
            </div>
