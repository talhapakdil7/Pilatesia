...
export default function MyReservations() {
  ...
  const isLessonStarted = (startTime) => {
    if (!startTime) return true;
    const s = String(startTime).trim().replace(" ", "T").slice(0, 16);
                  ...
                  {rows.map((r) => (
                    <tr key={r.reservation_id}>
                      ...
                      <td className="text-end">
                        {isLessonStarted(r.lesson?.start_time) ? (
                          <span className="text-muted small">Ders başladı, iptal edilemez</span>
                        ) : (
                          <button
