...
- name: studios
description: Stüdyo oluşturma ve stüdyo kontrol işlemleri
- name: lessons
description: Kullanıcıların görebildiği ders işlemleri
- name: reservations
description: Ders rezervasyon işlemleri
- name: membership
description: Kullanıcının üyelik ve kredi bilgileri
- name: admin-lessons
description: Stüdyo admininin ders yönetimi
- name: admin-users
description: Stüdyo admininin kullanıcı yönetimi
- name: warmups
description: Isınma hareketleri yönetimi
...
paths:
  "/register-studio":
    post:
      tags:
      - auth
      summary: Register Studio
      description: Yeni stüdyo oluşturur ve ilk admin kullanıcıyı ekler (herkese açık).
      operationId: register_studio_register_studio_post
      requestBody:
        content:
          application/json:
            schema:
              "$ref": "#/components/schemas/StudioRegister"
        required: true
      responses:
        '200':
  ...
  "/studios/check":
    get:
      tags:
      - auth
      summary: Check Studio Code
      description: Stüdyo kodu geçerli mi kontrol eder (kayıt formu için).
      operationId: check_studio_code_studios_check_get
      parameters:
      - name: studio_code
        in: query
        required: true
